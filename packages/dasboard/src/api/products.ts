...
export async function getProducts(page = 1, limit = 10, search?: string): Promise<PaginatedProductsResponse> {
  try {
    const params = {
      page,
      limit,
...
export async function updateProduct(id: string, productData: UpdateProductDto): Promise<Product> {
  try {
    ...
    const response = await bffApi.patch(`/dashboard/products/${id}`, productData);
    return response.data;
  } catch (error) {
